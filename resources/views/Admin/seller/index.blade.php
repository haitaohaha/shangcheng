                ...
                <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info">
                    ...
                    @foreach($data as $k=>$v)
                        <tr>
                            <td>{{ $v->sid}}</td>
                            <td>{{ $v->sname }}</td>
                            <td>{{ config('lyg_status.status')[$v->status] }}</td>
                            <td>
                                <a href=" @if($v->status!=5) {{url('admin/seller/'.$v->sid)}} @else {{url("#")}} @endif "  title="商家详情" style="color:#000;font-size:20px;margin-right:40px;"   >
                                <input type="hidden" name="_method" value="put">商家详情</a>
